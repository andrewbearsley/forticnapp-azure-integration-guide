...
This deployment guide covers the end-to-end integration of an Azure environment with FortiCNAPP. The integration enables continuous posture assessment, identity-aware risk analysis, agentless workload scanning, data security posture management, secrets detection, and unified risk consolidation across Azure subscriptions.

The guide is organised in phases. Most pilots start with Phase 1 (core integration), then layer agentless scanning, DSPM, secrets, and alert forwarding once the baseline is operating.

### Capabilities enabled by phase

| Phase | Setup step | Capabilities enabled |
|---|---|---|
| 1 | Core Azure integration (Config + Activity Log) | CSPM, CIEM, Attack Path Analysis, Compliance reporting, Unified risk console |
| 2 | Agentless workload scanning | VM CVE scanning (stopped/running), package inventory, secrets-on-disk detection |
| 3 | DSPM | Sensitive data discovery and classification across Storage Accounts and blob containers |
| 4 | FortiGate Security Fabric integration | Network exposure context in workload risk scoring |
| 5 | Alert channels | Outbound alert forwarding (Splunk, ServiceNow, Teams, email, webhooks) |

Each phase below is independently deployable. Capabilities from earlier phases are not blocked by later phases.

---

## Phase 1 — Core Azure Integration
...
| Compliance frameworks expected | CIS Azure, NIST, ISO, plus any custom mapping required |
...
### Step 1.3 — Path A: Console wizard

The wizard runs Terraform under the hood (the same modules described in Path B). It bundles app registration, role assignment, Storage Account + Event Hub creation, Activity Log diagnostic setting, and the FortiCNAPP integration into a two-step UI.

Best suited to POCs, single-subscription deployments, and environments where the team has full Owner + Privileged Role Administrator access and doesn't want to introduce Terraform tooling. Less suited to enterprise environments where the deployment tenant's directory role policy typically blocks the Automated method's pre-requisites.
...
1. Sign in to FortiCNAPP at `https://<account>.lacework.net`
...
### Step 1.3 — Path B: Terraform

Same mechanism the wizard runs internally — Terraform applying the `lacework/config/azure`, `lacework/activity-log/azure`, and `lacework/ad-application/azure` modules. The standard path for enterprise and regulated environments.
...
## Phase 2 — Agentless Workload Scanning
...
- **Via the Phase 1 wizard** — tick **Agentless Workload Scanning** on Step 1 and provide regions + monitored subscription IDs on Step 2. The wizard deploys a default agentless setup using FortiCNAPP-managed defaults for VNet, NAT, and scheduling. Good for POCs and standard environments.
...
- Custom VNet supported — use an existing VNet/subnet instead of creating new ones
...
## Phase 3 — Data Security Posture Management (DSPM)
...
DSPM is enabled per Azure cloud-account integration once Phase 1 is in place.

1. In the FortiCNAPP console, navigate to **Settings → Integrations → Cloud Accounts**
2. Open the Azure integration created in Phase 1
...
## Phase 4 — FortiGate Security Fabric Integration
...
|---|---|---|
| CSPM (continuous misconfig detection) | Phase 1 (Config) | All subscriptions under the integration scope |
| CIEM (identity over-privilege) | Phase 1 (Config) | Cloud-only Entra ID supported, PIM-gated roles respected |
| Attack Path Analysis | Phase 1 (Config) + Phase 2 (Agentless) | Multi-hop combining misconfig + identity + CVE + network |
| Compliance reporting | Phase 1 (Config) | CIS, PCI, HIPAA, NIST, ISO 27001, SOC 2 out of the box. Custom frameworks supported. |
| VM CVE scanning | Phase 2 (Agentless) | Includes stopped/offline VMs via disk snapshot |
| Secrets detection (disk) | Phase 2 (Agentless) | Part of the agentless scanning job |
| Secrets detection (blob storage) | Phase 3 (DSPM) | Enabled per Azure integration |
| DSPM | Phase 3 (DSPM) | Storage Accounts + blob containers |
| FortiGate exposure context | Phase 4 (FortiGate) | Network reachability enriches risk scores |
| Unified risk console | All phases | Single portal consolidates findings from every source |
| SIEM forwarding | Phase 5 (Alert channels) | Splunk, ServiceNow, Teams, email, webhooks |
...
Once Phase 1 + Phase 2 data is present, FortiCNAPP automatically constructs attack-path graphs. No additional configuration is needed. Paths surface under **Attack Path Analysis** in the console and are factored into prioritised risk lists.
